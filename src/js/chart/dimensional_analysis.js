...
TimeSeries.dimensionalAnalysis = (function(){
    ...
    var createChartSelectBox = function (dashboard_id, chart_id, series_id, chart_selector, count, mode) {
        var dashboards_list = document.createElement('div'),
            ...
            // list_name = document.createElement("span"),
        ...
        series_list.addEventListener("click", function () {
            ...
            console.log(document.querySelector(".scotch-modal").offsetHeight, " >>>");
    ...
    var seeDimensionalAnalysis = function (dashboard_id, chart_id, series_id, chart_selector, div_id, panel_no, series_attributes) {
        var user_div = document.getElementById(div_id),
            ...
            main_DA_container,
            reset_all_button;
            // chart_description_div;

        // createGranularity({ selector: chart_selector + "_dimension_data",dateColumnName: chart_options.dateColumnName },data);
        if(user_div && !document.getElementById(div_id + "_dimensional_analysis_holder")) {
            // highlight_view =  document.createElement("div");
            chart_holder = document.createElement("div");
    ...
    var createCharts = function(dashboard_id, chart_id, series_id, options, parent_id, series_attributes) {
        var parent_container =  document.getElementById(parent_id + "_dimensional_analysis_holder"),
            ...
            // count = parseInt(document.getElementById(parent_id).getAttribute("panel-no"));
                        ...
                        .on("click", function(d, i) {
                            ...
                            if (next_empty_panel) {
                                var compare_suggestions = document.querySelector("#DA_panel_" + next_empty_panel + " #suggestions_div #compare_suggestion"),
                                    ...
                                    original_key = clicked_object[+panel_id].original_key;
                                    // if(series_name.seriesColumnName) {
                                        series_name = series_name.series;
                                    // }
                                // compare_suggestions.innerHTML = "";
...
// Suggestions:Do you want to compare how Week 54 of <<__>> is performing against another week? OR Select another chart > and series >
            ...
            panel_body = modalElements(chart_id, chart, series);
    ...
    var modalElements = function (chart_id, chart, series) {
        var modal_container = document.createElement('div'),
            dashboard_div = document.createElement('div'),
            dashboards_list = document.createElement('select'),
            first_option = document.createElement('option'),
            rest_of_option,
            chart_list = document.createElement("select"),
            chart_div = document.createElement('div'),
            // first_element = document.createElement("li"),
            chart_option,
            series_list = document.createElement("ul"),
            series_div = document.createElement('div'),
            // series_first_option = document.createElement("li"),
            allCharts = TimeSeries.allCharts,
            count = chart_id.split("DA_panel_")[1],
            ...
            span,
            filters_div = document.createElement("div"),
            filters_span = document.createElement('div');

        filters_div.className = 'comcharts-TS-DA-modal-filters';
        ...
        filters_span.className = 'suggestions-div-title filters-div-title';
        span = document.createElement('span');
        span.className = 'comcharts-TS-content-sub-heading comcharts-TS-suggestions-div-select-dashboard';
        span.innerHTML = 'Filters';
        filters_span.appendChild(span);

        dashboard_span.className = 'suggestions-div-title comcharts-TS-modal-filters-dropdown-title';
        span = document.createElement("span");
        span.className = 'comcharts-TS-content-sub-heading comcharts-TS-suggestions-div-select-dashboard comcharts-TS-modal-filters-dropdown-title';
        span.innerHTML = "Dashboards";
        dashboard_span.appendChild(span);

        dashboards_list.id = "panel_DA_dashboard_"+ count + "_select_box";
        dashboards_list.className = 'comcharts-TS-sidebar-ul comcharts-TS-sidebar-dropdown';
        first_option.innerHTML = 'dashboard';
        dashboards_list.appendChild(first_option);

        rest_of_options = document.createElement('option');
        rest_of_options.className = 'comcharts-TS-sub-category-feature-modal active';
        rest_of_options.dataset.value = "All";
        rest_of_options.dataset.displayvalue = "All";
        ...
        for (var i=0,len_i=all_dashboards.length ; i<len_i ; i++) {
            ...
            rest_of_options.dataset.value = all_dashboards[i];
            rest_of_options.dataset.displayvalue = TimeSeries.chart_options.menu.dashboards[0].list[all_dashboards[i]].subCategoryText;
            rest_of_options.innerHTML = TimeSeries.chart_options.menu.dashboards[0].list[all_dashboards[i]].subCategoryText;
            dashboards_list.appendChild(rest_of_options);

            var child_nodes = document.querySelectorAll('#'+all_dashboards[i]+' .chart-div');
            for (var j=0,len_j=child_nodes.length ; j<len_j ; j++) {
                var temp_chart = child_nodes[j].id,
                    chart_options = TimeSeries.chart_options[temp_chart],
                ...
                if (chart_options.isTrendSavedChart) {
                    continue;
                }
                // chart_option = document.createElement("li");
                chart_option = document.createElement("option");
                chart_option.className = 'comcharts-TS-sub-category-feature-modal';
                chart_option.value = temp_chart;
                chart_option.dataset.value = temp_chart;
                chart_option.dataset.displayvalue = chart_options.caption;
                chart_option.dataset.dashboard = all_dashboards[i];
                chart_option.dataset.displaydashboard = TimeSeries.chart_options.menu.dashboards[0].list[all_dashboards[i]].subCategoryText;
                chart_option.innerHTML = chart_options.caption || "Filter by time";
                chart_list.appendChild(chart_option);
                ...
                for (var k = 0; k < metric_columns_length; k++) {
                    ...
                    if(typeof metric_columns[k] == 'object') {
                        series_options.dataset.metric = metric_columns[k].metric;
                        series_options.dataset.seriesColumnName = metric_columns[k].seriesColumnName;
                        series_options.dataset.seriesName = metric_columns[k].seriesName;
                    ...
                    series_options.dataset.dashboard = all_dashboards[i];
                    series_options.dataset.chart = temp_chart;
                    series_list.appendChild(series_options);
                }
            }
        }
        //mimicking the behaviour of a drop down.......
        dashboards_list.onfocus = function() { this.options[0].style.display = "none"; };
        dashboards_list.onblur = function () { this.options[0].style.display = "block"; };
        ...
        chart_list.style["max-width"] = "100%";
        ...
        series_list.addEventListener("click",function () {
            ...
            var dashboard_id = event.srcElement.dataset.dashboard,
                chart_id = event.srcElement.dataset.chart,
                series_id,// = event.srcElement.dataset,
                series_attributes = event.srcElement.dataset,
                // child_nodes = document.querySelectorAll('#'+chart_id+' .chart-div'),
                selected_chart = event.srcElement.innerHTML;

            // if(series_id.seriesColumnName) {
    ...
    var daOnDrop = function (target_selector) {
        ...
        renderDA("DA_panel_1",true, "chart1", "Surface Pressure");
    };
